...
class HotelBronController {
    ...
    async updateOneHotelBron(req, res) {
        ...
        try {
            const updatedTour = await HotelBronService.updateOneHotelBron(id, tourData);
    ...
    async getOneHotelBron(req, res) {
        try {
            const getOneHotelBron = await HotelBronService.getOneHotelBron(req.params.id)
    ...
    async deleteHotelBron(req, res) {
        try {
            const deleteHotelBron = await HotelBronService.deleteHotelBron(req.params.id)
    ...
    async deleteAllHotelBrons(req, res) {
        try {
            const deleteAllHotelBrons = await HotelBronService.deleteAllHotelBrons(req.params.id)
    ...
    async updateHotelBronOrder(req, res) {
        ...
        try {
            for (let i = 0; i < orderedIds.length; i++) {
                ...
                await HotelBron.findByIdAndUpdate(id, { order: i + 1 }, { new: true, runValidators: true });
            }
