...
class AuthorTourController {
    ...
    async updateOneAuthorTour(req, res) {
        ...
        try {
            const updatedTour = await AuthorTourService.updateOneAuthorTour(id, tourData, photoPaths);
    ...
    async updateAuthorTourOrder(req, res) {
        ...
        try {
            for (let i = 0; i < orderedIds.length; i++) {
                ...
                await AuthorTour.findByIdAndUpdate(id, { order: i + 1 }, { new: true, runValidators: true });
            }
    ...
    async getOneAuthorTour(req, res) {
        try {
            const getOneAuthorTour = await AuthorTourService.getOneAuthorTour(req.params.id)
    ...
    async deleteAuthorTour(req, res) {
        try {
            const deleteAuthorTour = await AuthorTourService.deleteAuthorTour(req.params.id)
